...
export const CMD_HELP: Record<string, string[]> = {
  ...
  search: ["/search [position] — the admissible next closures from a QuCalc position, via the QLF QuCalc Search service.",
           "No position cuts the room along peers' /qlf-action proposals (a concurrent search, one set of listeners).",
           "Position: symbolic ^v<>/\\+-, @lemma, or cap:token. Flags: --depth N · --limit N · --events|--possibilities · --full.",
           "The search is the room's shared experiment — it asks the substrate which possibilities close from here — and its result is broadcast.",
           "/search url <endpoint> sets the service (python3 qucalc_search.py --serve); /search info shows its version and caps."],
...
export const SLASH_COMMANDS: SlashCmd[] = [
  ...
  { name: "ice",     template: "/ice ",       desc: "how peers connect: list · test · turn · stun · reset" },
