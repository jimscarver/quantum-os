...
const POWERBOX: PowerboxEntry[] = [
  ...
  { name: "stdoutAck", urn: "rho:io:stdoutAck", sig: "stdoutAck!(value, *return)",
    returns: "an ack once it has printed — stdout with a way to sequence after it" },
  { name: "stderrAck", urn: "rho:io:stderrAck", sig: "stderrAck!(value, *return)",
    returns: "an ack once it has printed" },

  { name: "sha256Hash", urn: "rho:crypto:sha256Hash", sig: "sha256Hash!(bytes, *return)",
    returns: "the digest as bytes" },
  { name: "keccak256Hash", urn: "rho:crypto:keccak256Hash", sig: "keccak256Hash!(bytes, *return)",
    returns: "the digest as bytes" },
  { name: "blake2b256Hash", urn: "rho:crypto:blake2b256Hash", sig: "blake2b256Hash!(bytes, *return)",
    returns: "the digest as bytes — the hash a deploy signature is taken over" },
  { name: "ed25519Verify", urn: "rho:crypto:ed25519Verify", sig: "ed25519Verify!(data, sig, pubKey, *return)",
    returns: "Bool" },
  { name: "secp256k1Verify", urn: "rho:crypto:secp256k1Verify", sig: "secp256k1Verify!(data, sig, pubKey, *return)",
    returns: "Bool — the curve RChain deploys are signed with" },

  { name: "insertSigned", urn: "rho:registry:insertSigned:secp256k1",
    sig: "insertSigned!((nonce, data), *deployerId, *return)",
    returns: "the uri, or Nil",
    note: "the nonce must be a number; unlike insertArbitrary this binds the entry to its signer, so only they can update it",
    deployOnly: true },
  { name: "registryOps", urn: "rho:registry:ops", sig: "registryOps!(op, arg, *return)",
    returns: "per operation" },

  { name: "pos", urn: "rho:rchain:pos", sig: "pos!(method, [args…])",
    returns: "per method",
    note: "proof of stake: bond [deployerId, amount, ret] · withdraw [deployerId, ret] · getActiveValidators [ret] · getBonds [ret]" },
  { name: "multiSigRevVault", urn: "rho:rchain:multiSigRevVault", sig: "multiSigRevVault!(method, [args…])",
    returns: "per method — the multi-signature counterpart of revVault" },
  { name: "deployerIdOps", urn: "rho:rchain:deployerId:ops", sig: "deployerIdOps!(op, arg, *return)",
    returns: "per operation — operations on a deployerId, e.g. its public key" },
  { name: "sysAuthToken", urn: "sys:authToken:ops", sig: "sysAuthToken!(op, arg, *return)",
    returns: "per operation — the system authority token; genesis contracts hold it, ordinary deploys do not" },
