...
| `/gov <sub>` | **Liquid-democracy governance** (rgov on primitives) — `new <name>`, `show`, `member add\|remove`, `issue <title>`, `delegate <peer> [on <issue>]`/`undelegate`, `vote <issue> \| opts [ranked]`, `treasury declare\|grant\|balance`, `kudos <m> <n>\|balance`, `status`, `list`. Transitive (global + per-issue) delegation + weighted tally; treasury/kudos via `/note`; see `Governance.md` |
