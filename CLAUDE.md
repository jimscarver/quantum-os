...
### RChain capability macros (`/global` — `global-macros.js` + `global.ts` + `global-agent.mjs`)

Bridges a room to an RChain chain: a room's state is ephemeral, a deploy is not. Full reference in [RChain_Macros.md](RChain_Macros.md).
