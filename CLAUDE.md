...
**`/rholang` is the current command** (`packages/browser/src/rholang.ts`). Three verbs over the node's HTTP API: `status`, `eval` (exploratory deploy — runs read-only over finalized state, so pure rholang returns values but the node's system processes bind without ever replying), and `deploy` (secp256k1 over blake2b256 of the protobuf encoding of `DeployDataProto` — the encoding must match the node's byte for byte, proto3 default-omission included). Multi-line capture: `eval`/`deploy` take every line typed after them until an empty one. Every program is wrapped in `new return, stdout, zfa, grant, verify, fuse in { … }`; a deploy additionally forwards `return` onto a public name and to stdout, and the browser reads that name back once the block is produced, because a deploy otherwise answers only into the node's log. `scripts/localnet/` runs a node to develop against. **A deploy cannot execute on a local node yet** — genesis funds the rholang RevVault contract while pre-charge reads native vault state ([rchain-rust#3](https://github.com/rchain-community/rchain-rust/issues/3)).
