...
**The search is the experiment, not a lookup.** All admissible histories exist a priori as possibility; the enumeration asks the substrate which of them close *from here*. `mode=events` makes that literal — a closure *is* an event, so each branch is reported only at its first closure. `mode=possibilities` is every closure within `max_depth`.
