...
| `/record` (⏺ in the toolbar) | Record what is on your screen, with audio, to a `.webm` download. `getDisplayMedia` + your mic, mixed through an `AudioContext`; the room's other voices come out of the page, so they are on it only if the picker's **Share tab audio** is ticked (a tab share, and not offered on Linux for a whole-screen capture). 15 fps / 1.2 Mbps ≈ 540 MB/hour. Needs no call in progress — it captures the screen, so it keeps whatever you had up. Broadcasts a `record {on}` line on start and stop: nobody is recorded silently |
