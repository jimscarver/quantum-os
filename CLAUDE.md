...
**`/rholang` is the current command** (`packages/browser/src/rholang.ts`). Three verbs over the node's HTTP API: `status`, `eval` (exploratory deploy — runs read-only over finalized state; pure rholang and the system processes both return values), and `deploy` (secp256k1 over blake2b256 of the protobuf encoding of `DeployDataProto` — the encoding must match the node's byte for byte, proto3 default-omission included). `eval`/`deploy` open a syntax-highlighted, live-linted editor (`rholang-editor.ts`) — Ctrl+Enter runs, Esc cancels, and it loads a `.rho` from disk, accepts one dropped on it, or saves the program back out; a program written inline runs as typed. Every program is wrapped in `new return, stdout, zfa, grant, verify, fuse in { … }`; a deploy additionally forwards `return` onto a public name and to stdout, and the browser reads that name back once the block is produced, because a deploy otherwise answers only into the node's log. `scripts/localnet/` runs a node to develop against. A deploy executes and is charged there, and both `eval` and `deploy` reach the qucalc powerbox — verified against rchain-rust `dev` at `f3f4759e9`. **`match` is broken in the node** ([rchain-rust#10](https://github.com/rchain-community/rchain-rust/issues/10)): branches are tried in reverse written order, so the last matching branch wins, and a `_` wildcard never matches at all. Nothing errors — the wrong branch runs, or none. That reaches every recursive contract, whose base case is idiomatically a `match` arm, and a catch-all that wins at the base case recurses forever, which [#11](https://github.com/rchain-community/rchain-rust/issues/11) turns into node death: exploratory deploy has no phlo ceiling, so an unbounded term overflows the stack and aborts the process. Everything else works. Several sends to `return` all come back but in no dependable order (`eval` reverses source order; the deploy read path differs) — encode order in the program if it matters.

**`/global` is deprecated**, pending a redesign around programs rather than argument lists. Full reference in [RChain_Macros.md](RChain_Macros.md).

**The body is rholang, not a command line.** `/global` takes a rholang program — one line or many — with macro call sites written `%name(arg, …)`. The rholang is **not parsed**: `expandProgram` scans only far enough to find call sites that are really call sites (skips string literals and both comment forms, balances `()[]{}`,  splits args on top-level commas), expands each in place, and passes every other byte through as written. The `%` sigil is what makes that safe without a grammar — a bare `ballot(…)` is indistinguishable from a real contract call. Errors never abort: every site is attempted, each error carries its line, and a failed site is left exactly as typed. The bare form (`/global transfer 100 bob`) still works — the whole program is one macro.

**Single source of truth.** `packages/browser/src/global-macros.js` is plain JS with no imports, consumed by **both** halves — `scripts/qos-cli/global-macros.mjs` (18-line binding, node) and `packages/browser/src/global.ts` (browser). The ZFA kernel is *injected* (`createMacroEngine(kernel)`) because each side has its own build (`zfa.mjs` / `zfa.ts`). They were once separate copies; a macro edited in one and not the other meant the rholang a user reviewed in chat was not the rholang their browser signed, and both copies independently carried the same `Number()` precision bug.
...
- **Reliability**: `qospeer.mjs` has a 30s ping/pong **keepalive** (and a 30s `"disconnected"` teardown grace — see the pegged-CPU fix above) (reconnects a dead/zombie signaling socket — without it a long-running agent goes deaf to new joiners). Persistent identity + greet-state under `--state ./.qos-<role>`. **`run-agents.sh` / `stop-agents.sh`** launch/stop a detached set — `facilitator` + `skeptic`, staggered 15s (`STAGGER=n`); `/global` is deprecated and no longer launched. **`room-memory.mjs`** holds the durable half of a room (lemmas, currencies, terms-series, gov groups, dyncap chains, transcript; re-served to every joiner) and has two carriers: `qos-daemon.mjs` as a peer that does nothing else (no AI, no subscription), or `agent.mjs --persist <dir>` folding it into a role agent — one fewer peer against the signaling ceiling, at the cost of tying durability to an agent that also talks. Serving is deliberately **not** lead-gated: greeting is lead-only so N agents don't all greet, but state must be served by whoever holds it or a joiner gets nothing. `scribe` is not launched by default because its duties are a strict subset of the facilitator's; `skeptic` is, because it alone carries `verify`. `stop-agents.sh` with no arguments stops everything holding a pidfile rather than a hardcoded role list — the old list omitted `global`, so `global` could not be stopped by name and outlived every "stop all" around it. The free signaling server rate-limits the WebRTC offer/answer/ICE exchange itself, so past a handful of peers no data channel opens while everyone still appears in the room: measured by total peers in the room (an observer counts as one) 7 → 0 of 6 channels, 5 → 1 of 4, 4 → 3 of 3, and the default four interconnect. Add roles deliberately; your own signaling server removes the ceiling. **Self-healing identity announce:** the agent signs its `{name, agent:<role>}` envelope **once and caches it** (`announceName`/`signedNameEnv`), so the channel-open announce and the throttled re-announce from the 30s tick loop (`reannounceStale`, ≤ once/90s per present peer) are byte-identical idempotent re-deliveries (same anchor+seq+witness — no seq advance, no fork). This recovers the single channel-open announce when it is lost to a signaling flap (async sign + a closing data channel), which otherwise leaves a fresh browser showing the agent as an unlabelled hex id.
...
| `packages/browser/src/global.ts` | Browser half of `/global` — WASM lint, passphrase-wrapped key store (IndexedDB), sign, deploy. Binds the shared engine |
