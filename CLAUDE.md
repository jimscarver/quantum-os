...
- **Endpoint config** (`qos-qucalc-config` localStorage, `{url}`, no default): `/search url <endpoint>` sets it, `/search info` shows the service's version + caps.
...
- **Deploy target: the QLF repo.** The service depends on `twist_core.py`, so it is deployed from [`quantum-logical-framework`](https://github.com/rchain-community/quantum-logical-framework) (`python3 qucalc_search.py --serve`), not as a quantum-os sidecar. quantum-os is a pure consumer — point `/search url` at the running endpoint. Host ceiling: `--max-depth-cap 6 --max-concurrent 2` (depth 6 ≈ 3 s; depth 7 ≈ 1–2 s on a real box, where "~10 K possibilities" lands). The service is open and read-only; if exposure beyond the mesh is a concern, front it with the existing capability-token check rather than adding auth to the service.
...
| `/search [pos]` | The admissible **next closures** from a QuCalc position, via the QLF QuCalc Search service — *the search is the experiment* (truth divination). `pos` = symbolic / `@lemma` / `cap:token`; no arg = a concurrent search over the room's `/qlf-action` proposals (a *meeting of minds*). Each discovered event is saved as an integer-named room lemma (`@1`, `@2`, …); `--no-save`/`--save-cap N` control that. Flags `--depth`/`--limit`/`--events`/`--possibilities`/`--full`. `/search url <endpoint>` · `/search info`. Result is broadcast |
