...
**`/rholang` is the current command** (`packages/browser/src/rholang.ts`). Three verbs over the node's HTTP API: `status`, `eval` (exploratory deploy — runs read-only over finalized state; pure rholang and the system processes both return values), and `deploy` (secp256k1 over blake2b256 of the protobuf encoding of `DeployDataProto` — the encoding must match the node's byte for byte, proto3 default-omission included). Multi-line capture: `eval`/`deploy` take every line typed after them until an empty one. Every program is wrapped in `new return, stdout, zfa, grant, verify, fuse in { … }`; a deploy additionally forwards `return` onto a public name and to stdout, and the browser reads that name back once the block is produced, because a deploy otherwise answers only into the node's log. `scripts/localnet/` runs a node to develop against. A deploy executes and is charged there, and both `eval` and `deploy` reach the qucalc powerbox — verified against rchain-rust `dev` at `f3f4759e9`.
