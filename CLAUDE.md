...
| `/conn` | What each peer connection is actually doing — channel, connection and ICE state per peer. The roster says reachable or not; this says *why* not: `ice checking` forever is candidates that never pair, `failed` is no path at all, `connected` with no channel is a third fault. Local only |
