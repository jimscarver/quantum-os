...
| `scripts/localnet/macro-check.mjs` | **Are the macros still current?** Runs every expansion on a live rnode and reports per macro. There is no versioning between the library and rnode, so this is how an outdated macro is found — a failure names it, shows what was sent and what came back, and points at the definition. Fix the macro, then update its case. Not in CI (it needs a node) |
