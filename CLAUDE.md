...
| `packages/browser/src/display.ts` | Which pane the screen picker opens on — `preferredSurface`/`rememberSurface`, persisted `qos-share-surface`, shared by the recorder and the call's screen share. No API can preselect a *monitor*; opening where you shared last is the most a picker can do |
