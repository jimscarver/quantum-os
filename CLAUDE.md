...
**`reconcile`/`reconcilePrune`** (peer.ts's `"peers"`/`"joined"`/`"left"` handlers and `sweep()` all call it) arms a grace timer (`PRUNE_GRACE_MS`, 30s — between `DISCONNECT_GRACE_MS`'s 8s for a real ICE blip and `RETRY_MAX_MS`'s 90s backoff ceiling, giving a roster reshuffle a couple of `SWEEP_MS` cycles to settle) on any open connection that's fallen outside `targetPeers()`, cancelling it if the peer comes back into range first. On fire, **`prunePeer`** closes the link — **not** `declarePeerGone`: the peer usually hasn't left the room, just stopped being a direct neighbor, so it must never fire `onPeerLeft` (app.ts's leave handling assumes true departure — clears the display name, posts "X left"). It doesn't need extra guarding against that: `cleanup()` already empties `connections`/`channels` synchronously before the data channel's own `onclose` can fire, so `declarePeerGone`'s existing `had`-idempotency check finds nothing left to report by the time that async event lands (qospeer.mjs's channel close/state handlers don't report a departure at all, so this concern doesn't even arise there).
