...
| `/search [pos]` | The admissible **next closures** from a QuCalc position, via the QLF QuCalc Search service. `pos` = symbolic / `@lemma` / `cap:token`; no arg = a concurrent search over the room's `/qlf-action` proposals. Flags `--depth`/`--limit`/`--events`/`--possibilities`/`--full`. `/search url <endpoint>` · `/search info`. Result is broadcast (the room's shared experiment) |
